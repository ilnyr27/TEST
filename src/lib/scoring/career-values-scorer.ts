...
import { careerValuesDimensions } from "@/lib/test-engine/career-values-data";

const DIMENSIONS = ["ACH", "SEC", "IND", "HLP", "CRE"] as const;

export class CareerValuesScorer implements Scorer {
  calculate(
    questions: { id: string; scoring_key: { dimension: string; reverse?: boolean } | null }[],
    answers: Record<string, AnswerData>,
    locale: "ru" | "en"
  ): ScoringResult {
    const sums: Record<string, number[]> = { ACH: [], SEC: [], IND: [], HLP: [], CRE: [] };
    ...
    const dimensions = DIMENSIONS.map((dim) => {
      ...
      const meta = careerValuesDimensions[dim];
  ...
  private buildSummary(dims: { key: string; score: number }[], locale: "ru" | "en"): string {
    ...
    const top1 = sorted[0];
    const top2 = sorted[1];
    const meta = careerValuesDimensions as Record<string, { nameRu: string; nameEn: string }>;
    ...
    if (locale === "ru") {
      return `Ваши главные карьерные ценности — ${top1Name} (${top1.score}%) и ${top2Name} (${top2.score}%).`;
    }
    return `Your top career values are ${top1Name} (${top1.score}%) and ${top2Name} (${top2.score}%).`;
  }
}
